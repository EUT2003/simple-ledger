...
fun SettingsScreen(state: AppState, onBack: () -> Unit) {
    ...
    Surface(Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background) { Column(Modifier.fillMaxSize().padding(20.dp)) {
        ...
        ListItem(modifier=Modifier.clickable { dialog.value="day" }, headlineContent = { Text(t("每月起始日","Monthly start day")) }, supportingContent = { Text("${state.settings.monthStartDay} ${t("日","day")}") })
